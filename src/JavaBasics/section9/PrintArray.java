    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < inputStr.length() && validInput; i++) {
            ...
            if (!Character.isDigit(currChar)                                    // Invalidating if the string contains any
                && currChar != ' '                                              // character other than a number, a space 
                && currChar != '-')                                             // or a 'minus' sign
                
                validInput = false;
            
            if (i > 0                                                           // Invalidating if the string contains two  spaces
                && (currChar == ' '                                             // in a row
                && inputStr.charAt(i-1) == ' ') )
                validInput = false;
